...
/**************************私有方法区*****************************************/
    ...
    private String generateAccessPrivilege(List<String> roleNames) {
        ...
        String privilegeAuthc = "authc,";

        if (roleNames == null || roleNames.isEmpty()) {
            return "authc";
        } else if (roleNames.size() == 1) {
            String privilegeRole = "roles[";//角色等于一个为"authc,roles[admin]"格式
            privilegeRole = privilegeRole + roleNames.get(0) + "]";
            finalPrivilege = privilegeAuthc + privilegeRole;

            return finalPrivilege;
        } else {
            //生成权限
            String privilegeRole = "roleOr[";

            for (int i = 0; i < roleNames.size() - 1; i++) {
                privilegeRole += roleNames.get(i);
                privilegeRole += ",";
            }

            privilegeRole += roleNames.get(roleNames.size() - 1);
            privilegeRole += "]";
            finalPrivilege = privilegeAuthc + privilegeRole;
